...
//
type Count<T, P, K extends unknown[] = []> = T extends [infer HEAD, ...infer TAIL] ?
  HEAD extends P
    ? Count<TAIL, P, [...K, HEAD]>
    : Count<TAIL, P, K> :
  K['length']
...
type Flat<T> =
  T extends [infer HEAD, ...infer TAIL] ?
    HEAD extends unknown[] ? [...Flat<HEAD>, ...Flat<TAIL>] :
        [HEAD, ...Flat<TAIL>] :
    T

type CountElementNumberToObject<T extends unknown[], K extends any[] = Flat<T>> = {
  [P in K[number]]: Count<K, P>
}
type Case1 = CountElementNumberToObject<[1, 2, 3, 4, 5]>
//    ^?

...
type First1<T extends number[]> = T[0]
type ExcludeFirst<T extends number[]> = T extends [unknown, ...infer TAIL] ? TAIL : []

type FirstArray<M extends number[][]> = M extends [infer HEAD extends number[], ...infer TAIL extends number[][]] ? [First1<HEAD>, ...FirstArray<TAIL>] : []
type F1 = FirstArray<[[1], [3], [5]]>
//    ^?
type ExcludeFirstArray<T extends number[][]> = T extends [infer HEAD extends number[], ...infer TAIL extends number[][]] ? [ExcludeFirst<HEAD>, ...ExcludeFirstArray<TAIL>] : []
type F2 = ExcludeFirstArray<[[1], [3], [5]]>
//    ^?

type Transpose<M extends number[][]> = M extends [][] ? [] : [FirstArray<M>, ...Transpose<ExcludeFirstArray<M>>]
type C1 = Transpose<[[1, 4], [2, 5], [3, 6]]>
//    ^?
